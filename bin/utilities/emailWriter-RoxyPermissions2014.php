...
// Stuff to be modified for each new email to be generated:
      ...
      SSFWebPageParts::setPageHeaderTitleText('Permission Request for Roxy, 12/2014');   
      ...
      $subjectTemplate = 'Requesting permission to screen "<title>" on <shortDate>';
                 
      $emailTemplate = 'Dear <name>,<br><br>'
                 . 'Congratulations on having "<title>" selected to screen on <longDate> at <venue>. '
                 . 'This screening is billed as "Kinetoscope at the Roxy" and is cosponsored by Bare Bait Dance (http://barebaitdance.org/).<br><br>'
                 . 'Sans Souci Festival of Dance Cinema requests your permission to screen "<title>” at this venue. Please respond to grant or deny such permission.<br><br>'
                 . 'Thanks very much.<br><br>'
                 . 'Best wishes,<br><br>'
                 . "Hamel Bloom<br>Executive Director<br>Sans Souci Festival of Dance Cinema<br>http://sanssoucifest.org<br>";
...
<?php
  ...
  echo SSFWebPageParts::beginContentHeader();
    ...
    function getEmailBodyText($title, $submitterName) {
      global $emailTemplate, $date, $longDate, $shortDate, $venue;
      $bodyText = str_ireplace('<title>', $title, $emailTemplate);
...
//      $bodyText = str_ireplace('<br>', PHP_EOL, $bodyText);
    ...
    $query = 'SELECT title, name, email FROM works JOIN runOfShow ON work=workId JOIN shows ON `show`=showId JOIN people ON submitter=personId '
           . 'WHERE SUBSTR(permissionsAtSubmission,1,5)="askMe" AND ' . $queryWhereClause;
