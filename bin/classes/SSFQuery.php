...
class SSFQuery {
  ...
  private static $lastUpdateFields = array();
  private static $lastWorkContributorsUpdateFields = array();
      ...
      foreach (self::$curatorRows as $curatorRow) {   
        ...
        if (!isset($curationIndices[$workId][$curator])) {
          $insertString = "INSERT INTO curation (entry, curator, lastModificationDate, lastModifiedBy) VALUES (" 
                        . $workId . ", " . $curator . ", '" . SSFRunTimeValues::nowForDB() . "', " . $administrator . ")";
          //SSFDB::debugNextQuery();
    ...
    if (isset($sortIndexValue) && ($sortIndexValue != '')) {
      switch ($sortIndexValue) {
        ...
        case "durationSort":
          ...
          break;
        case "scoreSortUp":
          self::$querySortString .= $querySortStringPrefix . " avgScore ASC"; // changed from " avg(score) ASC" 7/4/12
          self::$querySortDisplayText .= $sortTextPrefix . "Average Score";
          break;
        case "scoreSortDn":
          self::$querySortString .= $querySortStringPrefix . " avgScore DESC"; // changed from " avg(score) DESC" 7/4/12
          self::$querySortDisplayText .= $sortTextPrefix . "Average Score";
          break;
        ...
        case "submitterSort":
          ...
          break;
        case "countrySort":
          self::$querySortString .= $querySortStringPrefix . " countryOfProduction ASC, country ASC";
          self::$querySortDisplayText .= $sortTextPrefix . "Country of Production";
          break;
        case "acceptedSort":
...
*/
  ...
  public static function selectSubmitterAndWorkWithARCommsFor($workId, $debug=false) {
    $submitterWorkSelectString = 'SELECT ' 
       ...
       . 'photoLocation, photoURL, invited, acceptFor, aspectRatio, anamorphic, frameWidthInPixels, frameHeightInPixels, '
...
//    $creatorId = ((isset($stateArray['adminSelector']) && $stateArray['adminSelector'] != 0) ? $stateArray['adminSelector'] 6/11/11
    ...
    if ($hasDateCreatedCol && $hasCreatedByCol) {
      ...
      $insertString .= $insertValueString . ", '" . SSFRunTimeValues::nowForDB() . "', " . $creatorId . ", '" . SSFRunTimeValues::nowForDB() . "', " . $creatorId . ")";
    } else {
      $insertString .= ", lastModificationDate, lastModifiedBy"  . ") VALUES (";
      $insertString .= $insertValueString . ", '" . SSFRunTimeValues::nowForDB() . "', " . $creatorId . ")";
    }
    //SSFDB::debugNextQuery();
...
//      $updatingUserId = isset($newValueArray['adminSelector']) ? $newValueArray['adminSelector'] 
      ...
      if ($valueChanged) {
        ...
        if ($otherRole) {
          ...
          self::$lastUpdateFields[] = 'workContributors_role_' . $roleKey;
        }
  ...
  public static function updateDBFor($tableName, $currentValueArray, $newValueArray, $whereKeyName, $whereKeyValue, $handleSetsAsStrings=false) {  
    ...
    foreach (DatumProperties::getColsForTable($tableName) as $colName) {
      ...
      $dpArray = DatumProperties::getArray();
      $dpObject = DatumProperties::forKey($dataItemKey);
$showItemInDebug = (($debugChanges == 1) && ($dataItemKey == 'people_notifyOf')) ? 1 : -1;                                           // 4/7/15
$newValueIsSet = isset($newValueArray[$dataItemKey]) ? 'TRUE' : 'FALSE';                                                             // 4/7/15
...
SSFDebug::globalDebugger()->becho('&nbsp;&nbsp;&nbsp;510. SSFQuery::updateDBFor newValue', $newValue, $showItemInDebug);             // 4/7/15
    ...
    if (count($updateArray) > 0) { // there are updates
      ...
      foreach ($updateArray as $updateCol => $updateValue) {
        ...
        $dataItemKey = DatumProperties::getItemKeyFor($tableName, $updateCol);
        self::$lastUpdateFields[] = $dataItemKey;
      }
...
//                          . $workId . " AND curator=" . $curatorId . " AND curators.isActive=1";
    ...
    if ($curationRecordExists) {
      ...
      $curatorInsertOrUpdateString = "INSERT INTO curation (curator, entry, score, notes, lastModificationDate, lastModifiedBy) "
                                   ...
                                   . SSFQuery::quote(str_replace("  ", " ", trim($notes)))
                                   . ", '" . SSFRunTimeValues::nowForDB() . "', " . $adminId . ")";
    }
    //SSFDB::debugNextQuery();
    $querySuccess = true;
    ...
    if ($curatorCount > 0) {
      ...
      if ($curationRowsToAddCount > 0) {
        ...
        foreach ($curatorsToAdd as $curatorToAdd) {
          ...
          $curationInsertString .= "(" . $workId . ", " . $curatorToAdd . ", '" . SSFRunTimeValues::nowForDB() . "', " . $administratorId . ")" . $valuesSeparator;
...
//                        . "WHERE (communications.type='AcceptReject' OR communications.type ='' OR communications.type is null) AND (sent=0 OR sent is null OR true)) AS commsSelected) "
                    ...
                    . "ON commsSelected.work=worksSelected.workId "
                    . "WHERE commsSelected.sent = 0 OR commsSelected.sent = '' OR commsSelected.sent is null";

/*      This was the code here prior to 7/8/12
        $finalQuery = "SELECT * FROM "
                    . "((" . $basicWorksQuery . ") AS worksSelected)" 
                    . " WHERE `work` NOT IN "
                    . "(SELECT `work` "
...
//    SSFDB::debugNextQuery();
  ...
  public static function selectAccRejWorkRow($workId) {
    $worksSelectString = "SELECT personId, name, lastName, organization, city, stateProvRegion, country, email, "
                       ...
                       . "photoLocation, photoURL, communications.communicationId, communications.type, communications.sent, "
                       . "communications.dateSent, communications.sender, communications.contentText, "
                       . "communications.emailTo, communications.emailFrom, communications.emailSubject, "
