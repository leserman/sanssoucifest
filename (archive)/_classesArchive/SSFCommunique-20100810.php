...
/* ??? */    . " LEFT JOIN communications on communication=communicationId" // changed from left join to join 7/15/10
    ...
    private static $worksSelectString = "SELECT title, workId, designatedId, submitter, accepted, rejected, acceptFor, dateMediaReceived, photoLocation, includesLivePerformance";
...
*/
    ...
    public function insertIntoDatabase($adminUserId) {
      ...
      if (count($this->potentiallyReferencedWorks) > 0) {
        ...
        if ($valuesClause == "") self::$debugger->bechoTrace('insertIntoDatabase valuesClause is empty', $adminUserId, 1);
...
//    public function workId() { return $this->potentiallyReferencedWorks[0]['workId']; }
    ...
    private static function displayWorkIds($constStyle, $workArray) {
      return "<td align='left' width='20%' style='" . $constStyle . "padding:0 0px 0 4px;'>" . $workArray['designatedId'] 
...
//      $displayText .= "<tr><td align='center'>Work Id</td><td align='center'>&nbsp;</td><td align='left'>Title</td><td align='center'>&nbsp;</td><td align='left'>Widget Id</td></tr>\r\n";
    ...
    private function storePermissionRequests($userId) {
      ...
      foreach($this->potentiallyReferencedWorks as $work) {
        if ($work['accepted'] == 1) {
          $insertQuery = "INSERT INTO `sanssouci`.`permissionRequest` "
                       . "(event, permissionType, requestComm, work, dateGenerated, lastModificationDate, lastModifiedBy) "
                       . "VALUES (" . SSFRunTimeValues::getCallForEntriesId() . ", 'DemoReelClip', " . $this->commId 
                       . ", " . $work['workId'] . ", '" . $this->dateSent . "', NOW(), " . $userId . ")";
          //SSFDB::debugNextQuery();
...
// AcceptReject Communique "subclass" // If there are any works for which the acceptance status is undecided, produce no letter but rather a note to the administrator
    ...
    public function acceptanceMessagePart1a() {
      $part = "Congratulations! It will be our pleasure to <screen> your <film>, <title>"
            . " <at our> Seventh Annual Sans Souci Festival of Dance Cinema.";
      return $part;
      }
    ...
    private function rejectionWithinAcceptanceMessage() {
    ...
    public function acceptanceMessagePart2() {
    ...
    public function acceptanceMessageClosing() {
      $closing = "Again, congratulations and thank you for making such compelling work. I look forward to seeing more of your work in the future.<br><br>";
      return $closing;
    }
    ...
    private function rejectionMessagePart2() {
      $part = "Regrettably we will be not presenting <title> as part of our festival this year. We had a large number of excellent"
            . " entries from all of over the world, so we had to make some very hard decisions.<br><br>";
      return $part;
    }
    
    public function inviteFeedbackRequest() { // Unused as of 7/24/10
    ...
    private function imageDetailPart() {
    ...
    private function atlasDirections() {
    ...
    private function installationExplanation() {
    ...
    private function newAcceptRejectMessageBody($requestClipPermission=true, $inviteFeedbackRequest=false) {
      ...
      // TODO: When the user has an option to omit some works, use only those works, not $this->potentiallyReferencedWorks.
      foreach($this->potentiallyReferencedWorks as $work) {
        $stillImagesNeeded = HTMLGen::stillImagesNeeded($work['photoLocation']);
        $thisOneForInstallation = stripos($work['acceptFor'], 'installation') !== false;
        ...
        if ($work['accepted'] == $work['rejected']) { $numberOfUndecidedWorks++;     $undecidedWorkTitles[] = $work['title']; }
          ...
          else                                      { $numberOfWorksToScreen++;      $screenWorkTitles[] = $work['title']; }
        }
      ...
      if ($totalNumberOfWorks == 0) {
        ...
        $message = 'There are no works selected. Thus, there is no email message. It may be that your browser cache and/or cookies got cleared.<br><br>'
                 ...
                 . '  3. Then, you can select any envelop in the list on the left and everything will be fine and dandy.<br>';
        ...
        if ($numberOfAcceptedWorks > 0) { // If any messages at all were accepted
          // ACCEPTED
          self::$debugger->belchTrace('newAcceptRejectMessageBody this->potentiallyReferencedWorks[0]', $this->potentiallyReferencedWorks[0], -1);
          // TODO Mention documentary vs installation vs screening
          ...
          if ($forInstallation && $numberOfWorksToScreen != 0) { 
          ...
          $part2 = str_replace('<title>', self::generateTitleString($rejectedWorkTitles, 'or'), $plugTheShow . $this->acceptanceMessagePart2());
          ...
          if ($requestClipPermission) {
            $msgPart = str_replace('<title>', self::generateTitleString($acceptedWorkTitles, 'and'), $this->clipRequest());
            $msgPart = str_replace('<clip>', ($numberOfAcceptedWorks == 1) ? 'a clip' : 'clips', $msgPart);
            $message .= $msgPart;
          }
          ...
          $msgPart2 = str_replace('<title>', self::generateTitleString($rejectedWorkTitles, 'or'), $this->rejectionMessagePart2());
        ...
        if ($explainInstallation) $message .= "<br><br>" . $this->installationExplanation();
        $message .= "<br><br>" . $this->atlasDirections();
        if ($numberOfWorksNeedingImages > 0) $message .= "<br><br>" . $this->imageDetailPart();
      }
