...
<?php
  ...
  function selectContributors2($workId, $allFixedOptional) {
    ...
    $selectContributorString = "SELECT work, contributorOrder, name, role FROM workContributors"
  ...
  function entrySummaryDisplay($workRow) {
    ...
    return "<i>" . clickableForDetailDisplay($workRow['workId'], $workRow['title']) . ",</i> " 
					...
					. "<span style='color:#b8ede0'>" . $workRow['submissionFormat'] . "</span> / " . originalFormatDisplay($workRow['originalFormat']) . "] <span style='color:#FFFF66'>" 
					. substr($workRow['runTime'], 1, 7) . "</span> " . acceptanceDisplay($workRow['accepted'],$workRow['rejected'])
...
//				 . "Submitter (Locale) [<span style='color:#b8ede0'>Submssn Format</span> / Orignl Format] <span style='color:#ffff4f'>Run Time</span>"
				 . "Submitter (Locale) [<span style='color:#b8ede0'>Submssn Format</span> / Orignl Format] <span style='color:#FFFF66'>Run Time</span>"
		...
		while ($worksQueryResult && ($workRow = mysql_fetch_array($worksQueryResult))) {
			echo "<tr><td align='right' valign='top' class='bodyTextOnDarkGray'>" 
			    ...
			    . "&nbsp;&nbsp;</td><td id='entryId-" . $workRow['workId'] . "' align='left' valign='top' class='bodyTextOnDarkGray'>"
